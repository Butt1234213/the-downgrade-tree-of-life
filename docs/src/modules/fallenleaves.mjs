...
document.addEventListener('DOMContentLoaded', (event) => {
	...
	fallenLeaves.marbled = new fallenType(
		...
		[
			...
			`<span class="bold">3 cap</span><br>Unlock the Forge`,
		],
		() => storage.gameData.fallLevel.gt(new Decimal(1)),
		'This Fallen Leaf type unlocks when you complete the Fall challenge.'
	);
	fallenLeaves.coal = new fallenType(
		...
		'Coal Leaves',
		4,
		() => [storage.rootUpgradeFactor.items.coal.amount],
		['log<sub>5</sub>(Coal)'],
		function(x) {return (Decimal.log(x.plus(new Decimal(1)), new Decimal(5))).trunc()},
